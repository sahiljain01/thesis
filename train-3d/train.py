...
def train(data_dir, bs, num_epochs, lr, dim, layers):
    logging.info("Starting training...")
    losses = []

    model = gin_k_layers.GIN(num_features=1, layers=layers)
    ...
    torch.save(bestModel.state_dict(), f"best_model-{bs}-{num_epochs}-{lr}-{layers}.pt")
    wandb.finish()
    return loss, None
...
if __name__ == "__main__":
    ...
    parser.add_argument('--architecture', type=str, default="GIN")
